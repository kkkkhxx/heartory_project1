...
${MARK_SHIPPED_MODAL}    (//*[@role='dialog'][.//*[self::h1 or self::h2][normalize-space(.)='Mark as shipped']])[1]
${ADD_TRACKING_BTN}        xpath=${MARK_SHIPPED_MODAL}//button[normalize-space(.)='Add tracking number']
${TRACKING_INPUT}          xpath=${MARK_SHIPPED_MODAL}//input[@type='text' or contains(translate(@placeholder,'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz'),'tracking')]
${SAVE_SHIPPED_BTN}        xpath=${MARK_SHIPPED_MODAL}//button[normalize-space(.)='Save']
${NOTIFY_TOGGLE_OPTIONAL}  xpath=${MARK_SHIPPED_MODAL}//*[@role='switch' or @aria-checked or contains(@class,'switch')]
...
Scroll To Fulfillment Section (Robust)
    ...
    # 1) รอให้ heading หรือปุ่มอย่างน้อยหนึ่งอย่างมาอยู่ใน DOM
    ...
    # 2) ถ้ามี section ให้เลื่อนไปหา section ก่อน
    ...
    # 3) ไม่มี section? ลองหา ancestor ของปุ่ม Mark as shipped มาเป็น section แทน
    ...
    Run Keyword If    ${has_btn} and ${has_anchor}    Run Keywords
    ...    Scroll Element Into View    ${_try_anchor}
    ...    AND    Sleep    200ms
    ...    AND    Execute Javascript    window.scrollBy(0, -80);
    ...    AND    Return From Keyword

    # 4) ถ้ายังไม่ได้จริง ๆ ให้เลื่อนไปที่ปุ่มโดยตรง (กันกรณีไม่มี wrapper section ที่ match)
    Run Keyword If    ${has_btn}    Run Keywords
    ...
    # 5) กัน virtualized UI: ค่อย ๆ เลื่อนลงทั้งหน้าให้ component render แล้วลองอีกครั้ง
    FOR    ${i}    IN RANGE    12
    ...
    # ถ้าจบ loop แล้วยังไม่เจอ ให้ fail พร้อม hint
...
# ---------- High-level flows สำหรับนำไปใช้ในเทส ----------
Part1_Fulfill_Order
    [Documentation]    Login -> Orders -> เปิด Not fulfilled -> Create Fulfillment -> กลับหน้า Orders
...
Part2_Add_Tracking_To_Fulfilled_Order
    [Documentation]    Login -> Orders -> เข้าออเดอร์ Fulfilled -> Mark as shipped -> ใส่ tracking -> ตรวจผล
    Admin Login    ${ADMIN_USER}    ${ADMIN_PASS}
    Admin Open Orders Page
    Admin Open First Fulfilled Order
    Scroll To Fulfillment Section (Robust)
    ...
    Fill Mark Shipped Modal And Save    AUTO    ${True}
