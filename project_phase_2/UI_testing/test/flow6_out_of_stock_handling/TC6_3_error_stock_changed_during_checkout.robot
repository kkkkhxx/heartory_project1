...
...                  (ในระบบจริงแก้ได้ผ่าน In stock → ใช้สูตร In stock = Reserved + Available ใหม่)
...               4) Customer พยายามซื้อจำนวน = Available ล่าสุด + 1 → ต้องขึ้น error ที่หน้า Cart ว่า inventory ไม่พอ
...
# ใช้ data-testid หรือ href ตรงเมนู Products
${ADMIN_MENU_PRODUCTS}      xpath=//a[@data-testid='nav-products-link' or contains(@href,'/products')]
...
*** Test Cases ***
TC6_3_Error_Stock_Changed_Between_Orders
    [Documentation]    รอบแรกซื้อ 3 ชิ้น → Admin ลด Available ลง 2 → รอบสองซื้อ (Available ปัจจุบัน + 1) ต้องขึ้น error
    # ----- Step 1: Admin อ่าน stock ก่อนเริ่ม -----
    Admin Login
    ${available_before}=    Admin Get Product Stock    ${PRODUCT_NAME}    ${LOCATION_NAME}
    Log To Console          [ADMIN] Available ปัจจุบัน (${LOCATION_NAME}) ก่อนเริ่ม = ${available_before}
    # ต้องมีสต็อกพอให้ซื้อ 3 แล้วเหลือ >= 2
    Should Be True          ${available_before} >= 5

    # ----- รอบแรก: ลูกค้าซื้อ FIX = 3 ชิ้น -----
    ${qty_first}=    Set Variable    3
    Log To Console   [TEST] รอบแรก ลูกค้าจะซื้อจำนวน ${qty_first} ชิ้น

    Customer Login As Eatburger
    Customer Buy Product With Quantity    ${PRODUCT_NAME}    ${qty_first}
    Log To Console    [CUSTOMER] ซื้อรอบแรกสำเร็จ จำนวน ${qty_first} ชิ้น

    # ----- Step 2: Admin อ่าน Available หลังซื้อรอบแรก -----
    Admin Login
    ${available_after_first}=    Admin Get Product Stock    ${PRODUCT_NAME}    ${LOCATION_NAME}
    Log To Console         [ADMIN] Available หลังซื้อรอบแรก (${LOCATION_NAME}) = ${available_after_first}
    Should Be True         ${available_after_first} >= 2

    # ----- Step 3: Admin ลด Available ลง 2 หน่วย -----
    ${new_available}=    Evaluate    ${available_after_first} - 2
    Log To Console       [ADMIN] ต้องการลด Available ของ ${LOCATION_NAME} จาก ${available_after_first} → ${new_available}
    Admin Set Stock For Location    ${PRODUCT_NAME}    ${LOCATION_NAME}    ${new_available}

    ${available_after}=    Admin Get Product Stock    ${PRODUCT_NAME}    ${LOCATION_NAME}
    Log To Console         [ADMIN] Available หลังปรับลด (${LOCATION_NAME}) = ${available_after}

    # ----- Step 4: รอบสอง ลูกค้าพยายามซื้อ (Available ล่าสุด + 1) → ต้อง error -----
    ${qty_second}=    Evaluate    ${available_after} + 1
    Log To Console    [CUSTOMER] พยายามซื้อรอบสอง จำนวน ${qty_second} ชิ้น (available ล่าสุด + 1) คาดหวังให้ fail

    Customer Login As Eatburger
    Customer Try Buy More Than Available Should Fail    ${PRODUCT_NAME}    ${qty_second}    ${OUT_OF_STOCK_ERROR}
...
Admin Set Stock For Location
    [Documentation]    ตั้งค่า "Available" ของ ${location_name} ให้เป็นค่า ${new_available} โดยคำนวณ In stock ใหม่จาก Reserved และรอจนหน้าอัปเดตจริง
    [Arguments]    ${product_name}    ${location_name}    ${new_available}

    # เปิดหน้า Inventory item
    Admin Open Inventory Item Detail    ${product_name}

    # อ่านค่า Reserved ปัจจุบันของ location
    ${reserved_cell}=    Set Variable
    ...    xpath=//table[.//th[normalize-space(.)='Location']]//tbody//tr[.//span[normalize-space(.)='${location_name}']]//td[2]//span
    Wait Until Element Is Visible    ${reserved_cell}    15s
    ${reserved_text}=    Get Text    ${reserved_cell}
    ${reserved}=         Convert To Integer    ${reserved_text}
    Log To Console       [ADMIN] Reserved ปัจจุบัน (${location_name}) = ${reserved}

    # คำนวณ In stock ใหม่ = Reserved + Available ใหม่
    ${new_in_stock}=    Evaluate    ${reserved} + ${new_available}
    Log To Console      [ADMIN] ตั้งค่า In stock ใหม่ = ${new_in_stock} (reserved=${reserved} + available=${new_available})

    # ปุ่มสามจุด (menu) ในแถว location
    ${menu_btn}=    Set Variable
    ...    xpath=//tr[.//span[normalize-space(.)='${location_name}']]//button[@aria-haspopup='menu']

    Wait Until Element Is Visible    ${menu_btn}    15s
    Click Element With Retry         ${menu_btn}
    Sleep    0.5s

    # เลือกเมนู Edit
    ${edit_item}=    Set Variable    xpath=//div[@role='menuitem' and normalize-space(.)='Edit']
    Wait Until Element Is Visible    ${edit_item}    10s
    Click Element With Retry         ${edit_item}

    # input จำนวน In stock ใหม่
    ${stock_input}=    Set Variable    xpath=//input[@type='number' and @name='stocked_quantity']
    Wait Until Element Is Visible    ${stock_input}    10s
    Clear Element Text               ${stock_input}
    Input Text                       ${stock_input}    ${new_in_stock}

    # Save
    ${save_btn}=    Set Variable    xpath=//button[@type='submit' and normalize-space(.)='Save']
    Click Element With Retry         ${save_btn}
    Sleep    1.5s

    # รอจน Available ในตารางอัปเดตเป็นค่าที่ต้องการจริง ๆ
    Wait Until Keyword Succeeds    5x    2s
    ...    Admin Location Available Should Be
    ...    ${product_name}    ${location_name}    ${new_available}

Admin Location Available Should Be
    [Arguments]    ${product_name}    ${location_name}    ${expected_available}
    ${actual}=    Admin Get Product Stock    ${product_name}    ${location_name}
    Log To Console    [ADMIN] Check Available (${location_name}) ตอนนี้ = ${actual}, expected = ${expected_available}
    Should Be Equal As Integers    ${actual}    ${expected_available}


# ======================= CUSTOMER =======================
Customer Login As Eatburger
    [Documentation]    ล็อกอิน eatburger; ถ้า login form ไม่ขึ้น แปลว่าอาจล็อกอินอยู่แล้ว → ข้ามการกรอก
    Go To    ${STORE_HOME_URL}
    Wait Until Element Is Visible    ${BTN_ACCOUNT_OR_LOGIN}
    Click Element With Retry         ${BTN_ACCOUNT_OR_LOGIN}

    ${login_form_visible}=    Run Keyword And Return Status
    ...    Wait Until Element Is Visible    ${LOGIN_EMAIL_INPUT}    5s

    IF    ${login_form_visible}
        Log To Console    [CUSTOMER] Login form visible → กรอก email/password
        ...
        Sleep    1.5s
    ELSE
        Log To Console    [CUSTOMER] ดูเหมือนล็อกอินอยู่แล้ว → ข้ามการกรอกฟอร์ม
        Sleep    1s
    END
...
Customer Try Buy More Than Available Should Fail
    ...
    # 4) เปลี่ยน quantity ให้เป็นค่าที่ต้องการ (ที่เราตั้งใจให้เกิน stock)
    Set Cart Quantity                ${qty}
    ...
    Log To Console    [CUSTOMER] Error message = ${msg}
